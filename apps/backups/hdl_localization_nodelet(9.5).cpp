...
float downsample_resolution = 0.9;
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr laserCloudFullResColor2(new pcl::PointCloud<pcl::PointXYZRGB>());

pcl::PointCloud<pcl::PointXYZRGB>::Ptr sublaserCloudFullResColor(new pcl::PointCloud<pcl::PointXYZRGB>());
...
private:
  ...
  void points_callback_(const sensor_msgs::PointCloud2ConstPtr& point_msgs_livox){
    ...
    // pcl::PointCloud<PointType>::Ptr pcl_cloud_livox(new pcl::PointCloud<PointType>());
    ...
    // auto filtered_livox = downsample_with_label(pcl_cloud_livox);
    ...
    if(!use_imu) {
      ...
      // auto aligned_livox_ = pose_estimator->correct_with_label(filtered_livox);
      ...
      *laserCloudFullRes2 = *aligned_livox_;
      ...
      // printf("the label of lasercloudfullres2: %d\n", laserCloudFullRes2->points[0].label);

      for (int i = 0; i < laserCloudFullResNum; i++) {
      //       // printf("the label of lasercloudfullres2: %d\n", laserCloudFullRes2->points[i].label);
...
void LabelpointAssociateToMap(PointType const * const pi, pcl::PointXYZRGB * const po)
{
  ...
  pcl::PointCloud<pcl::PointXYZL>::ConstPtr downsample_with_label(const pcl::PointCloud<pcl::PointXYZL>::ConstPtr& cloud) {

    pcl::PointCloud<pcl::PointXYZL>::Ptr filtered(new pcl::PointCloud<pcl::PointXYZL>());

    pcl::VoxelGrid<pcl::PointXYZL> downsample_filter;
    downsample_filter.setInputCloud (cloud);
    downsample_filter.setLeafSize (downsample_resolution, downsample_resolution, downsample_resolution);
    downsample_filter.filter (*filtered);
    filtered->header = cloud->header;
    NODELET_INFO("downsample_with_label!");
