...
public:
  ...
  void onInit() override {
    ...
    rslidar_sub = mt_nh.subscribe("/rslidar_points", 1, &HdlLocalizationNodelet::points_callback_, this);
    ...
    filter_pub = nh.advertise<sensor_msgs::PointCloud2>("/aligned_rslidar", 5, false);
...
private:
  ...
  void points_callback_ (const sensor_msgs::PointCloud2ConstPtr& points_msg_rslidar){
    std::lock_guard<std::mutex> estimator_lock(pose_estimator_mutex);

    const auto& stamp = points_msg_rslidar->header.stamp;

    NODELET_INFO("RECEIVED rslidar ^ ^");
    pcl::PointCloud<PointT>::Ptr pcl_cloud_rslidar(new pcl::PointCloud<PointT>());
    pcl::PointCloud<PointT>::Ptr pcl_cloud_rslidar_(new pcl::PointCloud<PointT>());

    // pcl::PointCloud<PointT>::Ptr pcl_cloud(new pcl::PointCloud<PointT>());
    pcl::fromROSMsg(*points_msg_rslidar, *pcl_cloud_rslidar);
    ...
    pcl::transformPointCloud (*pcl_cloud_rslidar, *pcl_cloud_rslidar_, transform_to_map *transform_1.inverse());

    auto filtered_rslidar = downsample(pcl_cloud_rslidar);
    // predict
    if(!use_imu) {
      ...
      auto aligned_rslidar_ = pose_estimator->correct(filtered_rslidar);
      ...
      sensor_msgs::PointCloud2 aligned_rslidar;
      pcl::toROSMsg(*aligned_rslidar_, aligned_rslidar);
      aligned_rslidar.header.frame_id = "map";
      filter_pub.publish(aligned_rslidar);
  }
  /**
